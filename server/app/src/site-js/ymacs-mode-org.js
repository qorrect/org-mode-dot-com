...
function expandAll(code, markers) {
    ...
    console.dir(markers);
    const new_lines = [];
    const lines = code.split(Strings.Defs.NEWLINE);
    ...
    return new_lines.join(Strings.Defs.NEWLINE);
}
...
Ymacs_Tokenizer.define('org', (stream, tok) => {
    ...
    function _do_indent(local_stream) {
        ...
        if (currentLine && currentLine.match(/^\*/)) {
            ...
            const found = local_stream.buffer.markers.filter(marker => marker.name === Strings.Defs.FOLDED_MARKER).some(marker => {
                ...
                if (markerLine === currentLineNumber) {
                    ...
                    let insertString = '\n' + code.join(Strings.Defs.NEWLINE);
            ...
            if (!found) {
                ...
                const foldedMarker = local_stream.buffer.createMarker(local_stream.buffer.caretMarker.getPosition(), true, Strings.Defs.FOLDED_MARKER);
...
(function () {
    ...
    Ymacs_Buffer.newCommands({
        ...
        org_expand_buffer: Ymacs_Interactive(function () {
            ...
            const markers = this.markers.filter(marker => marker.name === Strings.Defs.FOLDED_MARKER);
            const new_lines = expandAll(code, markers);
            this.setCode(new_lines);
            this.cmd('goto_char', pos);
            // Now remove all markers
            this.markers = this.markers.filter(marker => marker.name !== Strings.Defs.FOLDED_MARKER);
